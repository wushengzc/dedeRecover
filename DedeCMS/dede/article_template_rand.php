...
if($dopost=='save')
{
    ...
    foreach (explode(",", $cfg_disable_funs) as $value) {
     ...
     if(preg_match("#^[\s]+<\?(php|=)?[\s]+#i", " {$templates}") == TRUE) {
        if(preg_match("#[$][_0-9a-z]+[\s]*[(][\s\S]*[)][\s]*[;]#iU", " {$templates}") == TRUE) {
            $templates = dede_htmlspecialchars($templates);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$templates}</pre>");
        }
        if(preg_match("#[@][$][_0-9a-z]+[\s]*[(][\s\S]*[)]#iU", " {$templates}") == TRUE) {
            $templates = dede_htmlspecialchars($templates);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$templates}</pre>");
        }
        if(preg_match("#[`][\s\S]*[`]#i", " {$templates}") == TRUE) {
            $templates = dede_htmlspecialchars($templates);
            die("DedeCMS提示：当前页面中存在恶意代码！<pre>{$templates}</pre>");
        }
    }
